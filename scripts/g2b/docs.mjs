...
/* 해석기 판. 뽑아내는 규칙(require.mjs·doc 읽기·이 파일의 합치기)이 좋아지면
   여기를 올립니다 — 그래야 예전 판으로 읽어 둔 공고가 다시 읽힙니다.
   판을 안 올리면 코드를 아무리 고쳐도 이미 읽은 공고에는 영영 반영되지
   않습니다. 실제로 그 사고가 났습니다: 등급표 수정 두 번을 판 없이 내보내
   "밤하늘 캠핑" 공고가 옛 결과로 계속 남았습니다. 그래서 자체점검이
   해석기 파일의 지문(analyzer-stamp.json)을 대조해, 코드가 바뀌었는데
   판이 그대로면 실패하도록 되어 있습니다. */
const VERSION = 3;
